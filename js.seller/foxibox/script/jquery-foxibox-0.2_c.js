 ...
 (function($) {
	  function _getBasePath() {
			...
			for (var i = 0, len = els.length; i < len; i++) {
				src = els[i].src || '';
				if (/g\.js/.test(src)) {
					return src.substring(0, src.lastIndexOf('/') + 1) + 'foxibox/' ;
				}
			}
			return '';
		}
	  ...
	  function _loadStyle() {
			var head = document.getElementsByTagName('head')[0] || (document.compatMode != 'CSS1Compat' ? document.body : document.documentElement),
				url = _getBasePath() + 'style/jquery-foxibox-0.2.css';
		    ...
		    $(window).resize(function(){
		      ...
		      var element_href = $(element).attr('href');
		        ...
		        var this_href = $(images_array[i]).attr('href');
						if(this_rel && this_rel.indexOf(gallery) != -1 && this_rel == element_rel){
		          set_count++;
